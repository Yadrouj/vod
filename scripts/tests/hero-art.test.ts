...
test("hero prefers curated landscape art for chart titles with poster-only catalog data", () => {
  ...
  assert.match(art ?? "", /image\.tmdb\.org\/t\/p\/original/);
});
...
test("hero preserves original TMDB art for desktop", () => {
  const original = "https://image.tmdb.org/t/p/original/backdrop.jpg";
