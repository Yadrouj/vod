...
test("reviewed full-film references match catalogue identities without fabricated downloads", async () => {
 const entries: {id: string; title: string}[] = readLegacyList(await readFile("scripts/data/old-iranian-film-list.txt", "utf8"));
 const refs: {id: string; videoId: string; filmTitle: string; durationSeconds: number; playbackStatus: string}[] = JSON.parse(await readFile("scripts/data/old-iranian-video-reviewed.json", "utf8")).items;
 ...
 for (const ref of refs) {
  ...
  const title = JSON.parse(await readFile(`public/data/titles/${ref.id}.json`, "utf8")) as {youtubeVideos: {videoId: string}[]; links: {url: string}[]};
