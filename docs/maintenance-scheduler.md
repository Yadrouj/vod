...
## اجرای قابل‌اعتماد روی sarvnema.ir

روش پیشنهادی سرور اکنون **تایمر systemd میزبان** است: هر پنج دقیقه بررسی،
فعال پس از ریبوت، بدون اجرای هم‌زمان چند اسکریپر. بازهٔ پیش‌فرض همان
**۰۳:۰۰ تا قبل از ۰۷:۰۰ تهران** و تنظیمات ذخیره‌شدهٔ داشبورد محفوظ است.
پس از به‌روزرسانی کد روی سرور، یک بار اجرا کنید:
...
نصاب ابتدا ایمیج را می‌سازد، سپس daemon قدیمی را متوقف و تایمر را فعال می‌کند؛
دادهٔ آرشیو را پاک نمی‌کند. راهنمای کامل نصب، بررسی نتیجه، انتشار بعدی و توقف:
[راهنمای DevOps تایمر روزانه](../deploy/SCHEDULER.md).

بررسی ترافیک اکنون بر اساس درخواست‌های فعال و صف واقعی است؛ ۱۲ درخواست تاریخی
دیگر به‌تنهایی باعث توقف یک سایت سالم نمی‌شود، مگر این سقف را صریحاً تنظیم کنید.
محدودیت حافظه، بار میزبان و اتاق‌های فعال همچنان برقرار است. نتیجهٔ آخرین اجرای
شبانه در `data/maintenance-scheduler-last-run.json` حفظ می‌شود و با بررسی روزانه پاک نمی‌شود.

بخش‌های daemon و cron زیر، روش‌های جایگزین هستند؛ آن‌ها را همراه تایمر فعال نکنید.
...
**پشتیبان‌گیری:** این bind mountها داخل checkout هستند. قبل از deploy بعدی از آن‌ها بکاپ بگیرید؛ نسخهٔ جدید `deploy/deploy.sh` اگر دادهٔ محلی تغییر کرده باشد متوقف می‌شود و دیگر reset نمی‌کند. دادهٔ زنده را برای تمیزکردن Git حذف نکنید. به‌روزرسانی شبانه خودش git/build اجرا نمی‌کند.
...
# خالی: ظرفیت صف واقعی؛ مقدار عددی فقط برای سقف سخت‌گیرانهٔ ترافیک تاریخی
MAINTENANCE_MAX_RECENT_REQUESTS=
