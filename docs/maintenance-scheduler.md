...
در کار شبانهٔ موسیقی، پیش از ساخت indexهای کوچک، اسکریپت `enrich-music-artwork.mjs` حداکثر ۴۰ صفحهٔ منبع را با یک درخواست هم‌زمان بررسی می‌کند. فقط تصویر دارای تطبیق نام اثر، یا تصویر صفحهٔ اختصاصی یک ترک پذیرفته می‌شود؛ جلد عمومیِ مجموعه به تمام آهنگ‌ها نسبت داده نمی‌شود. هیچ فایل صوتی دانلود نمی‌شود.

نتایج و صفحات حل‌نشده در `data/music-artwork-journal.json` ثبت می‌شوند؛ فاصلهٔ بررسی مجدد ۷ روز است. پیدا شدن تصویر اختصاصی برای تمام آثار تضمین نمی‌شود. قفلِ بیش از ۱۲ ساعت، فقط در صورت نبودن پردازش فعال بازیابی می‌شود.

بررسی دستی محدود: `node scripts/enrich-music-artwork.mjs --limit=20`؛ پس از تغییر تصاویر، `node scripts/build-music-landing-index.mjs` را اجرا کنید. مرحلهٔ شبانه این ساخت را خودش انجام می‌دهد.
