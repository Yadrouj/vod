...
منوی جدید فیلم و سریال در هر دو بات از یک کنترل‌کنندهٔ مشترک
(`scripts/bot-navigation.mjs`) استفاده می‌کند:

```text
منوی اصلی ← فیلم/سریال/آرشیو منتخب
              ← تنظیمات ← ژانر/کشور/سال/IMDb/کیفیت/ترتیب ← بازگشت به تنظیمات
              ← نمایش نتایج ← عنوان ← فصل ← قسمت ← کیفیت و لینک
```

- انتخاب «همه / بدون فیلتر ژانر» فقط فیلتر ژانر را پاک می‌کند؛ کشور یا نتیجه‌ها را خودکار باز نمی‌کند.
- نمایش نتایج فقط با دکمهٔ مشخص کاربر انجام می‌شود. فیلترها اختیاری و قابل پاک‌کردن‌اند.
- گزینه‌ها و تعدادشان بر اساس همان بخش و فیلترهای دیگر محدود می‌شوند. سال‌ها از جدید به قدیم و صفحه‌بندی‌شده‌اند؛ تعداد آثار ترتیب سال را عوض نمی‌کند.
- بازگشت از عنوان، همان صفحهٔ نتایج یا جست‌وجو را باز می‌کند؛ بازگشت از قسمت نیز همان صفحهٔ قسمت‌ها را حفظ می‌کند.
- دکمهٔ هر منو به نسخهٔ همان منو متصل است؛ دکمهٔ قدیمی مقدار فیلتر دیگری را انتخاب نمی‌کند. پس از ری‌استارت بات، کاربر باید `/start` بزند؛ نشست‌ها در حافظه‌اند.
- جست‌وجوی `breakng bud` از همان تصحیح چندکلمه‌ای سایت استفاده می‌کند و `Breaking Bad` پیشنهاد می‌دهد. پیشنهادها قابل انتخاب‌اند؛ یک کلمهٔ مشترک به‌تنهایی کافی نیست.

تلگرام همچنین دکمهٔ Mini App و دستور `/app` دارد. راهنمای آدرس، BotFather و
انتشار وب‌اپ و workerها: [Mini App تلگرام](./telegram-mini-app.md).
