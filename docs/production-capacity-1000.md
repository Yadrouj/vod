...
## کشف خودکار فیلم و قسمت جدید

سرویس `catalog-sync` داخل Docker هر شش ساعت، مستقل از لپ‌تاپ توسعه‌دهنده، این زنجیره را اجرا می‌کند:

1. آدرس آرشیو دنیای سریال و میزبان‌های جایگزین را probe می‌کند.
2. تعداد عنوان و لینک را اعتبارسنجی می‌کند؛ افت غیرعادی باعث توقف می‌شود و کاتالوگ سالم قبلی دست‌نخورده می‌ماند.
3. عنوان‌های جدید و تغییر URL حتی با تعداد لینک یکسان را تشخیص می‌دهد؛ عنوان‌های غایب از پاسخ منبع حذف نمی‌شوند.
4. نوع، سال، ژانر، امتیاز و تعداد رأی را از دیتاست رسمی روزانه IMDb به‌روزرسانی می‌کند.
5. فقط عنوان‌های تازه را برای پوستر، تصویر، تریلر و بازیگران به سرویس متادیتای غنی می‌فرستد.
6. فایل هر عنوان، سرچ، صفحه اول و ایندکس بازیگران را در staging می‌سازد و در پایان به‌صورت atomic جایگزین می‌کند.

فاصله اجرا قابل تنظیم است:

```bash
VOD_SYNC_INTERVAL_MS=21600000       # هر ۶ ساعت
VOD_SYNC_INITIAL_DELAY_MS=90000     # ۹۰ ثانیه بعد از بالا آمدن سرویس
```

اجرای دستی و اجرای آزمایشی بدون تغییر داده production:

```bash
npm run sync-vod-catalog
VOD_SYNC_DRY_RUN=1 npm run sync-vod-catalog
```

آخرین نتیجه در `public/data/vod-sync-status.json` ثبت می‌شود. اپلیکیشن تغییر mtime ایندکس‌ها را بررسی می‌کند و حداکثر طی پنج دقیقه صفحات cache‌شده با داده جدید ساخته می‌شوند.
