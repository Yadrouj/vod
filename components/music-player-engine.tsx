...
export function MusicPlayerEngine({
  ...
  const [requestedId, setRequestedId] = useState(track.id);
  if (requestedId !== track.id) {
  ...
  if (previousLyricsKey !== lyricsKey) {
    setPreviousLyricsKey(lyricsKey);
  ...
  useEffect(() => {
    ...
    try { saved = window.localStorage.getItem(PLAYBACK_KEY); } catch { return; }
  ...
  function syncProgress() {
    ...
    try { window.localStorage.setItem(PLAYBACK_KEY, JSON.stringify({ id: activeTrack.id, time: player.currentTime, sourceUrl: source.url, updatedAt: Date.now() })); } catch { /* Playback works with storage disabled. */ }
  }
  ...
  function toggleLiked() {
    try {
      ...
      window.dispatchEvent(new Event("sarvnema-music-likes"));
    } catch {
      // Likes are optional when browser storage is unavailable.
    }
  }
  ...
  return (
    <section className={`music-player music-player-pro ${hasVideo ? "music-player-video" : "music-player-audio"}`} dir="auto">
      ...
      <div className="music-player-shell">
        ...
        {(() => { const downloadSource = activeTrack.sources.find((item) => item.kind === "download") ?? source; return <a className="music-download" href={downloadGateUrl({ url: downloadSource.url, musicId: activeTrack.id, title: activeTrack.title, quality: downloadSource.quality || downloadSource.label })} target="_blank" rel="noreferrer"><Download size={16} /> دانلود</a>; })()}
      </div>
...
function readLiked(id: string) {
  try { return new Set<string>(JSON.parse(localStorage.getItem(LIKES_KEY) || "[]")).has(id); } catch { return false; }
}
function subscribeLikes(callback: () => void) {
  window.addEventListener("sarvnema-music-likes", callback);
  window.addEventListener("storage", callback);
  return () => { window.removeEventListener("sarvnema-music-likes", callback); window.removeEventListener("storage", callback); };
}
