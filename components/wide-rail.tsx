...
export function WideRail({ items, locale }: { items: VodCard[]; locale: Locale }) {
  ...
  return <section className="section wide-rail-section">
    ...
    <div className="wide-rail">{order.slice(0, 6).map((item, index) => <Link key={item.imdbCode} className={`wide-card wide-card-${index % 3}`} href={`/${item.imdbCode}`} style={item.backdropUrl ? { backgroundImage: `url(${sizedImageUrl(item.backdropUrl, 720)})` } : undefined}><span className="rating">{item.imdbRating ? `IMDb ${item.imdbRating.toFixed(1)}` : item.year ?? t.common.movie}</span><span className="wide-copy"><strong>{localizedTitle(item, locale)}</strong><small>{[item.year, item.genres.slice(0, 2).join(" / ")].filter(Boolean).join(" / ")}</small><em>{t.home.wideAction}</em></span></Link>)}</div>
  </section>;
}
