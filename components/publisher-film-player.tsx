...
export function PublisherFilmPlayer({ source, title, poster }: { source: VodItem["publisherPlayer"]; title: string; poster?: string | null }) {
  ...
  return <section className="youtube-player-card" aria-label={`پخش ${title} از NFB`}>
    ...
    <div className="youtube-player-stage">{started ? <iframe title={`${title} · NFB`} src={player.embedUrl} allow="autoplay; fullscreen; picture-in-picture" allowFullScreen referrerPolicy="strict-origin-when-cross-origin" /> : <button type="button" className="youtube-player-poster" style={poster ? { backgroundImage: `url(${JSON.stringify(poster)})` } : undefined} onClick={() => setStarted(true)}><span className="youtube-play-control"><Play fill="currentColor" size={30} /></span><span className="youtube-poster-copy">باز کردن پلیر رسمی NFB</span></button>}</div>
