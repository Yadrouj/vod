  ...
  return <main className="shell music-public-playlist-page" dir="rtl">
    <section className="wrap">
      ...
      <article className="music-public-playlist-hero" style={firstTrack?.coverUrl ? { "--playlist-cover": `url("${firstTrack.coverUrl}")` } as CSSProperties : undefined}>
        ...
        <div className="music-public-playlist-copy"><p><Globe2 size={14} /> پلی‌لیست عمومی</p><h1>{playlist.title}</h1><span>{playlist.description || "یک میکس عمومی ساخته‌شده برای وقت‌هایی که انتخاب آهنگ سخت می‌شود."}</span><small><UserRound size={14} /> ساختهٔ {playlist.owner.name} · {playlist.trackIds.length.toLocaleString("fa-IR")} آهنگ</small><div><button className="music-public-play" type="button" onClick={() => document.getElementById("playlist-player")?.scrollIntoView({ behavior: "smooth", block: "center" })}><Play size={18} fill="currentColor" /> پخش پلی‌لیست</button><button className={starred ? "is-starred" : ""} type="button" onClick={() => void toggleStar()}><Star size={17} fill={starred ? "currentColor" : "none"} /> {playlist.starCount.toLocaleString("fa-IR")}</button><button type="button" onClick={() => void sharePlaylist()}><Share2 size={16} /> اشتراک</button></div></div>
      </article>
      ...
      {firstTrack ? <div id="playlist-player"><MusicPlayer track={firstTrack} queue={tracks.slice(1)} onTrackPlay={() => { void postPlaylistAction({ action: "engage", id: playlist.id, viewerId: getMusicCommunityProfile().id, kind: "play" }); }} /></div> : <div className="music-playlist-empty">آهنگ‌های این پلی‌لیست دیگر در آرشیو موجود نیستند.</div>}
