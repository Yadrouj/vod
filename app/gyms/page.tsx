    ...
    if (q)
      arr = arr.filter(
        (g) =>
          ...
          (g.nameEn?.toLowerCase().includes(q) ?? false) ||
          g.address.toLowerCase().includes(q)
      );
    if (userPos) {
      arr = [...arr]
        .map((g) => ({ g, d: distanceM(userPos, g) }))
        .sort((a, b) => a.d - b.d)
        .map((x) => x.g);
    }
    return arr;
      ...
      <div ref={listRef} className="mt-2 space-y-2">
        {shown.map((g) => (
          <GymRow
            key={g.id}
            gym={g}
            dist={userPos ? distanceM(userPos, g) : null}
            userPos={userPos}
            stat={stats[g.id]}
          />
        ))}
        {list.length === 0 && (
          <p className="py-8 text-center text-sm text-muted">{t("gym.noMatch")}</p>
        )}
